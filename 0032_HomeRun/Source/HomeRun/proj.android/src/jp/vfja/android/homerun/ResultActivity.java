...
public class ResultActivity extends SuperActivity{
	...
	private ButtonView resultTv, pointTv;
	...
	protected void initContentLayout() {
		...
		mPaint.getTextBounds(titleResult, 0, titleResult.length(), bounds);
		
		resultTv = new ButtonView(mContext, 
				300*Config.ratioX -bounds.width()-50 , 295*Config.ratioY, 
				bounds.height(), bounds.width()+2, titleResult, 
				Config.fontRanking, textSize, R.color.color_text_point_result);
		
		mPaint.getTextBounds(pointStr, 0, pointStr.length(), bounds);
		
		pointTv = new ButtonView(mContext, 
				550*Config.ratioX -bounds.width()-50 , 295*Config.ratioY, 
				bounds.height(), bounds.width()+2, pointStr, 
				Config.fontRanking, textSize, R.color.color_text_point_result);
		
		imgRankin = new Image(mContext, 
				 150*Config.ratioX, (Config.SCREEN_HEIGHT/2 - 150*Config.ratioX), 
		...
		imgBackground = new ImageManual(mContext, 0, 0, Config.SCREEN_HEIGHT,
		 ...
		 btnGoTop = new ButtonView2Status(mContext, 
				 Config.TOP_START_X, Config.SCREEN_HEIGHT/2 + 40 *Config.ratioX + 50, 
		 ...
		 btnRestart = new ButtonView2Status(mContext, 
				 Config.TOP_START_X, Config.SCREEN_HEIGHT/2 - 100*Config.ratioX + 50, 
		 ...
		 btnVitalify = new ButtonView2Status(mContext, 
				 Config.TOP_START_X,Config.SCREEN_HEIGHT/2 + 180 *Config.ratioX + 50, 
	...
	protected void loadContentLayout() {
		...
		parent.addView(resultTv);
		parent.addView(pointTv);
		viewClick();
		setUpIconLoader();
		...
		parent.setOnTouchListener(new OnTouchListener() {
			...
			public boolean onTouch(View v, MotionEvent event) {
				...
				case MotionEvent.ACTION_UP:
					if(btnRestart.isOnSize(x, y)){
						btnRestart.setTouched(false);
						Intent i = new Intent(getApplicationContext(), HomeRun.class);
						startActivity(i);
						finish();
					}
	...
	private void showNumberResultImage(String result) {
		...
		ButtonView txtResult = new ButtonView(mContext, 
				500*Config.ratioX -bounds.width()-50 , 295*Config.ratioY, 
